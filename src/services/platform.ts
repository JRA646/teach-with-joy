...
export async function getPermissions(role?: string): Promise<PermissionKey[]> {
  if (!role) return []
  const { data } = await supabase.from('platform_role_permissions').select('permission_key').eq('role_key', role)
  return (data || []).map(row => row.permission_key as PermissionKey)
}

export async function hasPermission(permission: PermissionKey, role?: string, known?: PermissionKey[]): Promise<boolean> { if (role === 'admin') return true; return (known || await getPermissions(role)).includes(permission) }
