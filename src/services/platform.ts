...
export async function getFeatureFlags(_role?: string): Promise<PlatformFeatureFlags> {
  ...
  const value = (data || []).reduce<PlatformFeatureFlags>((flags, row) => {
...
export async function getNavigationItems(role: string): Promise<NavigationItem[]> {
  ...
  return navigationResult.data
    .filter(item => (!item.permission_key || permissionSet.has(item.permission_key)) && (!item.feature_flag_key || flags[item.feature_flag_key] !== false))
    .map(item => ({
      key: item.item_key,
      label: item.label,
      path: item.path,
...
export async function getDashboardWidgets(role: string): Promise<DashboardWidgetDefinition[]> {
  ...
  const permissionSet = new Set(permissions)
  return result.data
    .filter(widget => (!widget.permission_key || permissionSet.has(widget.permission_key)) && (!widget.feature_flag_key || flags[widget.feature_flag_key] !== false))
    .map(widget => ({
...
export async function saveFeatureFlag(key: string, enabled: boolean, options: { description?: string } = {}) {
  ...
  if (!result.error) {
    ...
    await recordActivity('platform.feature_flag.updated', 'feature_flag', key, { key, enabled })
  }
  return result
}
...
export async function upsertNavigation(item: Partial<NavigationItem> & { role_key?: string; item_key?: string }) {
  ...
  if (!result.error) await recordActivity('platform.navigation.updated', 'navigation', item.item_key || item.key, { role: item.role_key || 'teacher' })
  return result
}
...
export async function upsertDashboardWidget(widget: Partial<DashboardWidgetDefinition> & { role_key?: string; widget_key?: string }) {
  ...
  if (!result.error) await recordActivity('platform.dashboard_widget.updated', 'dashboard_widget', widget.widget_key || widget.key, { role: widget.role_key || 'teacher' })
  return result
}
