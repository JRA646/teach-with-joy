...
const teacherNavigation: NavigationItem[] = [
  ['dashboard', 'Dashboard', '/', 'LayoutDashboard', 'TEACHING', 'dashboard.view'],
  ['programs', 'Students & Programs', '/program', 'GraduationCap', 'TEACHING', 'programs.view'],
  ['proposals', 'Lesson Proposals', '/proposals', 'CalendarClock', 'TEACHING', 'programs.view'],
  ['schedule', 'Schedule', '/schedule', 'CalendarDays', 'TEACHING', 'schedule.view'],
  ['attendance', 'Attendance', '/attendance', 'CalendarCheck', 'TEACHING', 'attendance.view'],
  ['messages', 'Messages', '/messages', 'MessageCircle', 'TEACHING', 'messages.view'],
  ['subjects', 'Subjects', '/subjects', 'BookOpen', 'TEACHING', 'subjects.view'],
  ['profile', 'Profile', '/profile', 'UserRound', 'ACCOUNT', 'profile.view'],
].map(([key, label, path, icon, section, permission], sortOrder) => ({
  key, label, path, icon, section, sortOrder, permission: permission as PermissionKey, enabled: true,
}))

const studentNavigation: NavigationItem[] = [
  ['dashboard', 'Dashboard', '/', 'LayoutDashboard', 'LEARNING', 'dashboard.view'],
  ['program', 'My Program', '/program', 'GraduationCap', 'LEARNING', 'programs.view'],
  ['proposals', 'Lesson Proposals', '/proposals', 'CalendarClock', 'LEARNING', 'programs.view'],
  ['schedule', 'Schedule', '/schedule', 'CalendarDays', 'LEARNING', 'schedule.view'],
  ['messages', 'Messages', '/messages', 'MessageCircle', 'LEARNING', 'messages.view'],
  ['profile', 'Profile', '/profile', 'UserRound', 'ACCOUNT', 'profile.view'],
].map(([key, label, path, icon, section, permission], sortOrder) => ({
  key, label, path, icon, section, sortOrder, permission: permission as PermissionKey, enabled: true,
}))

export const DEFAULT_NAVIGATION: Record<string, NavigationItem[]> = {
...
export function hasPermission(role: string | undefined, permission: PermissionKey): boolean {
  return !!role && (DEFAULT_PERMISSIONS[role]?.includes(permission) ?? false)
}

export function getNavigation(role: string | undefined, flags = DEFAULT_FEATURE_FLAGS): NavigationItem[] {
  const items = DEFAULT_NAVIGATION[role || ''] || []
  return items.filter((item) => item.enabled && (!item.permission || hasPermission(role, item.permission)) && (!item.featureFlag || flags[item.featureFlag] !== false))
}
