  ...
  return <div className="page">
    <div className="page-title">
      <div>
        ...
        <p>Manage your students, enrollments, 20-session programs, attendance, make-ups, and progress.</p>
      </div>
      <div className="program-toolbar">
        <button className="btn" onClick={() => { void loadData(); setRefreshKey(x => x + 1) }}><RefreshCw size={16} /> Refresh</button>
    ...
    <section className="panel teacher-student-list">
      <div className="panel-head">
        <div><h3><Users size={18} /> My students</h3><p className="panel-subtitle">Students are shown here even before they have a program. Create an enrollment when they are ready.</p></div>
      </div>
      {students.length ? <div className="teacher-student-grid">{students.map(student => {
        ...
        return <button key={student.id} className={`teacher-student-card ${program?.id === selectedEnrollmentId ? 'selected' : ''}`} onClick={() => program && setSelectedEnrollmentId(program.id)}>
          <div className="teacher-student-avatar"><GraduationCap size={18} /></div>
          <div className="teacher-student-info"><strong>{student.full_name || student.email}</strong><span>{student.email}</span></div>
          <span className={`program-status ${program ? 'success' : ''}`}>{program ? (program.contract_type === 'three_month' ? '3-Month' : 'Monthly') : 'No program'}</span>
        </button>
...
function CreateEnrollment({ students, teacherId, close, done }: { students: Profile[]; teacherId: string; close: () => void; done: () => void }) {
  ...
  return <div className="modal-backdrop">
    <div className="auth-modal">
      ...
      <p>Choose the student and contract. The teacher account will own the enrollment and its generated session ledger.</p>
      {error && <div className="form-error">{error}</div>}
      <form className="stack" onSubmit={submit}>
        ...
        <div className="program-notice">This creates <strong>{d.sessions} session records</strong> automatically. Student absence consumes a session; teacher absence and eligible postponements create make-up sessions.</div>
