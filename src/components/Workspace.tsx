...
export default function Workspace({ profile }: any) {
  ...
  const nav = profile.role === 'teacher'
    ? [['dashboard', 'Dashboard'], ['schedule', 'Schedule'], ['bookings', 'My Bookings'], ['subjects', 'Subjects'], ['profile', 'Profile']]
    : [['dashboard', 'Dashboard'], ['schedule', 'Schedule'], ['bookings', 'My Bookings'], ['profile', 'Profile']]
    ...
    <div className="app-shell">
      ...
      <div className="app-body">
        <aside className="sidebar">
          {nav.map(([id, label]) => (
            <button key={id} onClick={() => setPage(id)} className={page === id ? 'nav-item active' : 'nav-item'}>
              {id === 'dashboard' ? <LayoutDashboard /> : id === 'schedule' ? <CalendarDays /> : id === 'bookings' ? <CheckCircle2 /> : id === 'subjects' ? <BookOpen /> : <UserRound />}
              {label}
            </button>
          ))}
        ...
        <main className="main-content">
          {page === 'dashboard' && <Dashboard profile={profile} go={() => setPage('schedule')} />}
          {page === 'schedule' && <Schedule profile={profile} />}
          {page === 'bookings' && <Bookings profile={profile} />}
...
function Dashboard({ profile, go }: any) {
  ...
  async function load() {
    const f = profile.role === 'teacher' ? 'teacher_id' : 'student_id'
    const { data } = await supabase.from('bookings').select('*,subject:subjects(*)').eq(f, profile.id).order('starts_at')
    setItems(data || [])
  }
...
function Schedule({ profile }: any) {
  ...
  async function deleteSlot(slot: any) {
    if (slot.status === 'booked') {
      setError('Booked slots cannot be deleted. Cancel the booking first.')
      return
    }
...
function Subjects() {
  ...
  async function add(e: any) {
    ...
    setSaving(true)
    const { error: insertError } = await supabase.from('subjects').insert({ name: name.trim(), description: desc.trim(), is_active: true })
    setSaving(false)
    if (insertError) { setError(insertError.message); return }
    setName('')
    setDesc('')
  ...
  async function remove(subject: any) {
    if (!window.confirm(`Remove “${subject.name}” from the active subject list?`)) return
    setError('')
    const { error: deleteError } = await supabase.from('subjects').update({ is_active: false }).eq('id', subject.id)
