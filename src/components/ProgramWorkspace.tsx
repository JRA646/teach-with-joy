...
import { CalendarDays, ChevronLeft, ChevronRight, FileText, GraduationCap, RefreshCw } from 'lucide-react'
...
export default function ProgramWorkspace({ profile, enrollmentId }: { profile: any; enrollmentId?: string }) {
  ...
  const sessionByDate=useMemo(()=>{
    ...
    sessions.forEach(s=>{if(!s.scheduled_start)return;const key=new Date(s.scheduled_start).toDateString();map.set(key,[...(map.get(key)||[]),s])})
    return map
  },[sessions])
  ...
  return <div className="program-workspace">
    ...
    <section className="panel program-calendar-panel"><div className="panel-head program-calendar-head"><div><h3>Program calendar</h3><p className="panel-subtitle">Visual month-by-month schedule with status at a glance.</p></div><div className="program-view-toggle"><button className={view==='calendar'?'btn primary small':'btn small'} onClick={()=>setView('calendar')}><CalendarDays size={15}/>Calendar</button><button className={view==='ledger'?'btn primary small':'btn small'} onClick={()=>setView('ledger')}><FileText size={15}/>Ledger</button></div></div>{view==='calendar'?<><div className="program-calendar-toolbar"><div className="calendar-nav-group"><button className="icon-btn" aria-label="Previous month" onClick={()=>setCalendarDate(new Date(calendarDate.getFullYear(),calendarDate.getMonth()-1,1))}><ChevronLeft size={17}/></button><div><strong>{calendarDate.toLocaleString('en-US',{month:'long',year:'numeric'})}</strong><span>{sessions.filter(s=>s.scheduled_start).length} scheduled sessions</span></div><button className="icon-btn" aria-label="Next month" onClick={()=>setCalendarDate(new Date(calendarDate.getFullYear(),calendarDate.getMonth()+1,1))}><ChevronRight size={17}/></button></div><button className="btn small" onClick={()=>setCalendarDate(new Date())}>Today</button></div><div className="program-calendar-grid weekdays">{dayNames.map(day=><div key={day} className="calendar-weekday">{day}</div>)}</div><div className="program-calendar-grid">{calendarDays.map(day=>{const items=sessionByDate.get(day.toDateString())||[];const muted=day.getMonth()!==calendarDate.getMonth();const today=day.toDateString()===new Date().toDateString();return <div key={day.toISOString()} className={`calendar-day ${muted?'muted':''} ${today?'today':''}`}><div className="calendar-day-number">{day.getDate()}</div><div className="calendar-events">{items.slice(0,3).map(s=><button type="button" key={s.id} className={`calendar-event ${s.status} ${s.makeup_for?'makeup':''}`} onClick={()=>setSelectedSession(s)}><span>#{s.session_number}</span><strong>{s.scheduled_start?new Date(s.scheduled_start).toLocaleTimeString('en-US',{hour:'numeric',minute:'2-digit'}):'Unscheduled'}</strong><small>{s.makeup_for?'Make-up':statusLabel(s.status)}</small></button>)}{items.length>3&&<span className="calendar-more">+{items.length-3} more</span>}</div></div>})}</div><div className="calendar-legend"><span><i className="legend-dot scheduled"/>Scheduled</span><span><i className="legend-dot completed"/>Completed</span><span><i className="legend-dot absent"/>Absent</span><span><i className="legend-dot makeup"/>Make-up</span><span><i className="legend-dot holiday"/>Holiday/Postponed</span></div></>:<div className="session-ledger">{sessions.map(s=><SessionRow key={s.id} session={s} role={profile.role} onManage={()=>setSelectedSession(s)}/>)}</div>}</section>
