...
import { Bell, CalendarCheck, CalendarDays, GraduationCap, LayoutDashboard, LogOut, MessageCircle, UserRound, BookOpen } from 'lucide-react'
...
export default function TeacherWorkspace({ profile }: { profile: any }) {
  ...
  const nav = [
    ...
    ['programs', 'Students & Programs', GraduationCap],
  ...
  const navigate = (next: string) => {
    const paths: Record<string, string> = { dashboard: '/', programs: '/program', schedule: '/schedule', attendance: '/attendance', messages: '/messages', subjects: '/subjects', profile: '/profile' }
  ...
  useEffect(() => {
    ...
    void loadUnread()
    const channel = supabase.channel(`teacher-workspace:${profile.id}`)
  ...
  return <div className="app-shell">
    <header className="app-header">
      ...
      <div className="top-user">
        ...
        {profile.avatar_url ? <img className="user-avatar user-avatar-image" src={profile.avatar_url} alt="Profile"/> : <div className="user-avatar">{profile.full_name?.[0] || 'T'}</div>}
        <div><strong>{profile.full_name}</strong><small>Teacher</small></div>
      </div>
    </header>
    <div className="app-body">
      ...
      <main className="main-content">
        {page === 'dashboard' && <TeacherDashboard profile={profile} openPage={navigate}/>} 
        {page === 'programs' && <TeacherPrograms profile={profile}/>} 
        {page === 'schedule' && <TeacherSchedule profile={profile} openPage={navigate}/>} 
        {page === 'attendance' && <TeacherAttendance profile={profile}/>} 
        {page === 'messages' && <Messages profile={profile}/>} 
        {page === 'subjects' && <TeacherSubjects/>} 
